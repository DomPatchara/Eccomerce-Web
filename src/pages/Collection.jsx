...
const Collection = () => {
  ...
  const toggleCategory = (e) => {
    ...
    if (category.includes(e.target.value)) {
      setCategory(prev => prev.filter(item => item !== e.target.value)) // Remove ออก ถ้าถูกเลือกไว้อยู่แล้ว
    }
    else {
      setCategory(prev => [...prev, e.target.value])  // ถ้ายังให้แอดเพิ่ม
    }
    ...
    <div className='flex flex-col gap-1 sm:flex-row sm:gap-10 pt-10 border-t'>
      ...
      <div className='min-w-60'>
        <p onClick={()=>setShowFilter(!showFilter)} className='my-2 text-xl flex items-center cursor-pointer gap-2'>FILTERS
          <img src={assets.dropdown_icon} alt="arrow-icon" className={`h-3 sm:hidden ${showFilter ? 'rotate-90': ''}`} />
        </p>
